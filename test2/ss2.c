...
//     // Simple hash function: sum the ASCII values of the characters
//     unsigned int hash = 0;
//     while (*key) {
//         hash += *key++;
//     }
...
//     new_entry->key = strdup(key); // Duplicate the key to avoid potential issues
//     new_entry->value = value;
//     new_entry->next = NULL;

//     // Insert the entry into the hash table using separate chaining
//     if (hash_table->entries[index] == NULL) {
//         hash_table->entries[index] = new_entry;
//     } else {
//         // Insert at the beginning of the linked list
//         new_entry->next = hash_table->entries[index];
//         hash_table->entries[index] = new_entry;
//     }
// }
...
//     Entry *current = hash_table->entries[index];
//     Entry *prev = NULL;
...
//             if (prev == NULL) {
//                 // It's the first entry in the linked list
//                 hash_table->entries[index] = current->next;
//             } else {
//                 // It's not the first entry, update the previous entry's next pointer
//                 prev->next = current->next;
//             }

//             // Free memory for the key and the entry
//             free(current->key);
//             free(current);
//             return;
//         }
...
//     // Search the linked list at the given index for the key
//     Entry *current = hash_table->entries[index];
//     while (current != NULL) {
//         if (strcmp(current->key, key) == 0) {
...
//     // Free memory for each entry (including duplicated keys) and the hash table itself
//     for (int i = 0; i < TABLE_SIZE; ++i) {
//         Entry *current = hash_table->entries[i];
//         while (current != NULL) {
...
//                 int x=get(my_hash_table,buffer);
//                 if(x==0)
//                 {
//                     insert(my_hash_table,buffer,1);
//                     // pthread_mutex_unlock(&mutex);
//                     sem_post(&mySemaphore);
//                     sleep(10);
//                     char buffer_temp[MAX_CLIENT_PATH_LENGTH];
...
//                     delete(my_hash_table,buffer_temp);
//                     // pthread_mutex_unlock(&mutex);
//                 }
//                 else
//                 {
